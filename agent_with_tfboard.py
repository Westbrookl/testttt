...
G,node_attr,min,max= rd.make_weighted_random_graph(SNODE_SIZE, 0.5, "normal",1)
...
with tf.Session() as sess:
    ...
    for i in range(200):
        ...
        c_optimize,c_loss_summary,c_summary=cc.train(s, v, LAPLACIAN_TENSOR, td_error)
        #a=aa.train(s,v,LAPLACIAN_TENSOR,action_one_hot,td)
        a_objective,a_entropy,a_entropy_summary,a_optimize,a_summary=aa.train(s,v,LAPLACIAN_TENSOR,action_one_hot,new_td)
        print(str.format("actor objective:{0},{1}",a_objective,a_entropy))
        now = time.clock()
        print(now - start)
        start = time.clock()
        ENTROPY_WEIGHT=ENTROPY_WEIGHT/1.05
